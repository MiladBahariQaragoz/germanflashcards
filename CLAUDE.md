...
- **Admin tools (admin = `AUTHORIZED_CHAT_ID`).** Admin-only commands are guarded in
  the handler by `_is_admin` AND surfaced only in the admin's menu via a chat-scoped
  `set_my_commands(..., scope=BotCommandScopeChat(AUTHORIZED_CHAT_ID))` in `main.py`
  (the public menu omits them).
  - `/admin` lists every user with their current streak + due counts
    (`db.get_admin_overview`) and a 🗑 button each. Removal is a confirm step
    (`admin_rm:` → `admin_rmok:`/`admin_rmno`) → `db.remove_user` deletes the `users`
    doc, the `access_requests` doc, and all the user's progress docs in both domains,
    and `qm.drop_user` clears in-memory sessions. **Silent** — the removed user is
    never messaged; they can `/start` to request access again.
  - `/broadcast <message>` messages every registered user. It stashes the text in
    `context.user_data["pending_broadcast"]` and shows a preview with a confirm step
    (`broadcast_send`/`broadcast_cancel`) before anything is sent; delivery is
    sequential with per-user error handling and a sent/failed tally.
